...
- `opfunc` : a function that takes a single input (X), the point
             ...
             The function should also have two optional parameters defining the inputs and outputs to evalute on
- `x`      : the initial point
...
function optim.seboost(opfunc, x, config, state)
  ...
  --print(state.sesopLastX:size())
  state.itr = state.itr or 0
  state.itr = state.itr + 1

  local timer = torch.Timer()

  x,fx = config.optMethod(opfunc, x, config.optConfig) -- Apply regular optimization method, chaning the model directly
  ...
  for i = 1, anchorsSize-1 do
    --print(sesopIteration)
  ...
  if (newDir:norm() > eps) then
    ...
    state.dirs[{ {},state.dirIdx }]:copy(newDir)  --/ state.newDir:norm()
  else
  ...
  -- Update Momentum
  if momentum > 0 then
      state.dirs[{ {},momentumIdx }] = state.dirs[{ {},momentumIdx }]:mul(momentum) + state.dirs[{ {}, state.dirIdx }]--:mul(1-momentum)
  end
